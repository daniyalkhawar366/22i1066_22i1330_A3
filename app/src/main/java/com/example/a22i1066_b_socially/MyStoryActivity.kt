...
class MyStoryActivity : AppCompatActivity() {
    ...
    private fun setupProgressBars() {
        ...
        Log.d(TAG, "Setting up $totalStories progress bars")
        ...
        Log.d(TAG, "Created ${progressBars.size} progress bars")
    }

    private fun showStory(index: Int) {
        if (index < 0 || index >= stories.size) {
            Log.d(TAG, "No more stories, finishing")
            finish()
            return
        }

        Log.d(TAG, "Showing story $index of ${stories.size}")
        ...
        for (i in progressBars.indices) {
            val progress = when {
                i < index -> 100  // Completed stories
                i == index -> 0   // Current story starts at 0
                else -> 0         // Future stories
            }
            progressBars[i].progress = progress
            Log.d(TAG, "Progress bar $i: ${progressBars[i].progress}%")
        }
    ...
    private fun startStoryProgress() {
        ...
        if (currentStoryIndex >= progressBars.size) {
            Log.e(TAG, "Invalid index: $currentStoryIndex >= ${progressBars.size}")
            return
        }
        ...
        Log.d(TAG, "Starting progress for story $currentStoryIndex")
        ...
        progressRunnable = object : Runnable {
            override fun run() {
                ...
                if (progress >= 100) {
                    ...
                    Log.d(TAG, "Story $currentStoryIndex completed (100%)")
                    handler.postDelayed({
                        moveToNextStory()
                    }, 100)
                } else {
    ...
    private fun stopStoryProgress() {
        progressRunnable?.let {
            ...
            Log.d(TAG, "Stopped progress animation")
        }
    ...
    private fun moveToNextStory() {
        ...
        if (currentStoryIndex < stories.size - 1) {
            ...
            Log.d(TAG, "All stories completed")
            finish()
        }
    }
